...
import { processTablesInReport } from "./chart";
import fs from "fs";
...
async function finalReportGeneration(state: AgentStateType): Promise<Partial<AgentStateType>> {
  ...
  return {
    ...
    messages: [
      ...
      new AIMessage(`报告已生成，正在处理图表...`),
    ]
  }
}

async function insertReportCharts(state: AgentStateType): Promise<Partial<AgentStateType>> {
  console.log("[insert_report_charts] 在报告中插入图表");
  
  const model = getChatModel(config.final_report_model);
  let reportContent = state.final_report || "";
  ...
  try {
    ...
    return {
      ...state,
      messages: [
        new AIMessage(`图表处理失败，但报告已生成并保存。错误: ${error instanceof Error ? error.message : String(error)}`),
      ]
    }
  }
}
...
const deepResearcherBuilder = new StateGraph(AgentState)
  ...
  .addNode("insert_report_charts", insertReportCharts)
  ...
  .addEdge("final_report_generation", "insert_report_charts")
  .addEdge("insert_report_charts", END);
