...
### 2. 任务要求  
1. 首先判断用户提问是否与数据分析相关
   - 如果用户的提问和数据分析类问题无关，返回以下 JSON 格式：
   \`\`\`json
   {
     "needs_clarification": true,
     "askBackPrompt": "<先对用户输入内容进行友善回复>。我是一位专业的数据分析师，专门处理企业经营数据分析问题。您刚才的问题超出了我的专业范围。我可以帮您分析诸如：'今年销售额同比如何？'、'各地区业绩表现怎样？'、'产品销量趋势分析'等数据相关问题。请重新提出您的数据分析需求。"
   }
   \`\`\`
...
──────────────────
具体步骤
解析 积极影响项和消极影响项 中的项目描述键值对（如 _name(项目名称)...）与数值字段（如 diff(差值)...）。
...
• 格式要严格遵守 markdown 表格格式，不要有不必要的换行符。
研究结论 仅基于表格内数字：指出最大/最小、涨幅/跌幅最大等。
总结 用日常商业语言，禁止再出现表格以外的任何新数字。
...
每个步骤内的general_questions：不带同环比的单个问题及其数值答案
每个步骤内的yoymom_questions：带同环比的问题及其数值答案，以及造成同环比变化的因素的数值影响程度等信息
...
4. 研究计划回顾
将研究计划里的每个步骤，把每个步骤的general_questions数据和yoymom_questions数据问题，整合输出。输出时要求如下：
...
- 如果某个步骤下面的general_questions和yoymom_questions的结果为空，或可供撰写报告的素材太少，就如实说明，如“查数结果太少，跳过本步骤”。
...
export const echartOptionPrompt = (
  table: string 
) =>`请将以下markdown表格转换为合适的ECharts配置对象。
